...
public isolated client class Client {
    ...
    public isolated function init(ConnectionConfig config, string serviceUrl = "https://api.hubapi.com/crm/v3/objects/feedback_submissions") returns error? {
    ...
    resource isolated function post batch/read(BatchReadInputSimplePublicObjectId payload, map<string|string[]> headers = {}, *PostCrmV3ObjectsFeedbackSubmissionsBatchReadReadQueries queries) returns BatchResponseSimplePublicObject|BatchResponseSimplePublicObjectWithErrors|error {
    ...
    # + queries - Queries to be sent with the request 
    # + return - successful operation 
    resource isolated function get [string feedbackSubmissionId](map<string|string[]> headers = {}, *GetCrmV3ObjectsFeedbackSubmissionsFeedbackSubmissionIdGetByIdQueries queries) returns SimplePublicObjectWithAssociations|error {
        string resourcePath = string `/${getEncodedUri(feedbackSubmissionId)}`;
        map<anydata> headerValues = {...headers};
        if self.apiKeyConfig is ApiKeysConfig {
            headerValues["private-app-legacy"] = self.apiKeyConfig?.privateAppLegacy;
        }
    ...
    # + queries - Queries to be sent with the request 
    # + return - successful operation 
    resource isolated function patch [string feedbackSubmissionId](SimplePublicObjectInput payload, map<string|string[]> headers = {}, *PatchCrmV3ObjectsFeedbackSubmissionsFeedbackSubmissionIdQueries queries) returns SimplePublicObject|error {
        string resourcePath = string `/${getEncodedUri(feedbackSubmissionId)}`;
        map<anydata> headerValues = {...headers};
        if self.apiKeyConfig is ApiKeysConfig {
            headerValues["private-app-legacy"] = self.apiKeyConfig?.privateAppLegacy;
    ...
    # + queries - Queries to be sent with the request 
    # + return - successful operation 
    resource isolated function get .(map<string|string[]> headers = {}, *GetCrmV3ObjectsFeedbackSubmissionsGetPageQueries queries) returns CollectionResponseSimplePublicObjectWithAssociationsForwardPaging|error {
